              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {user.watchlist && user.watchlist.filter(item => item && item._id).length > 0 ? user.watchlist.filter(item => item && item._id).map(item => (
                  <Link to={`/auctions/${item._id}`} key={item._id} className="group flex gap-4 p-3 bg-gray-50 rounded-xl hover:bg-white border border-transparent hover:border-gray-200 transition-all shadow-sm">
                    <img src={getImageUrl(item.images?.[0])} onError={(e) => { e.target.onerror = null; e.target.src = 'https://images.unsplash.com/photo-1523170335258-f5ed11844a49?auto=format&fit=crop&q=80&w=400'; }} className="w-20 h-20 object-cover rounded-lg" alt={item.name || ''} />
                        ...
                        <tr key={bid._id} className="hover:bg-gray-50/50 transition-colors">
                          <td className="px-6 py-4">
                            <Link to={`/auctions/${bid.product._id}`} className="flex items-center gap-4">
                              <img src={getImageUrl(bid.product.images?.[0])} onError={(e) => { e.target.onerror = null; e.target.src = 'https://images.unsplash.com/photo-1523170335258-f5ed11844a49?auto=format&fit=crop&q=80&w=400'; }} className="w-10 h-10 rounded border object-cover" alt="" />
                ...
                {orders.length > 0 ? orders.map(order => (
                  <div key={order._id} className="p-5 border border-gray-100 rounded-3xl hover:border-blue-100 hover:shadow-xl hover:shadow-blue-500/5 transition-all group overflow-hidden relative">
                    <div className="flex gap-5 relative z-10">
                      <img src={getImageUrl(order.product?.images?.[0])} onError={(e) => { e.target.onerror = null; e.target.src = 'https://images.unsplash.com/photo-1523170335258-f5ed11844a49?auto=format&fit=crop&q=80&w=400'; }} className="w-20 h-20 object-cover rounded-2xl border border-gray-100" alt={order.product?.name || 'Order'} />
                ...
                {soldOrders.length > 0 ? soldOrders.map(order => (
                  <div key={order._id} className="p-6 border border-gray-100 rounded-[2rem] hover:border-green-100 hover:shadow-2xl hover:shadow-green-500/5 transition-all bg-gray-50/30">
                    <div className="flex gap-6 mb-6">
                      <img src={getImageUrl(order.product?.images?.[0])} onError={(e) => { e.target.onerror = null; e.target.src = 'https://images.unsplash.com/photo-1523170335258-f5ed11844a49?auto=format&fit=crop&q=80&w=400'; }} className="w-24 h-24 object-cover rounded-2xl border border-gray-100" alt={order.product?.name || 'Sale'} />
                      <div className="flex-1">
                ...
                {myListings.length > 0 ? myListings.map(item => (
                  <div key={item._id} className="group border border-gray-100 rounded-3xl overflow-hidden hover:shadow-2xl hover:shadow-blue-500/5 transition-all bg-white flex flex-col">
                    <div className="relative aspect-[4/3] overflow-hidden">
                      <img src={getImageUrl(item.images?.[0])} onError={(e) => { e.target.onerror = null; e.target.src = 'https://images.unsplash.com/photo-1523170335258-f5ed11844a49?auto=format&fit=crop&q=80&w=400'; }} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" alt={item.name || 'Listing'} />
